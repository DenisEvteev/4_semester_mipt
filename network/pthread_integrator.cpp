...
void solve_problem(const bound_t& bound, double& result_per_machine, int threds){
	...
	double step = (bound.finish - bound.start) / static_cast<double>(threds);
	pthread_t* ids = new pthread_t[threds];

	int ret;
	for (int thr = 0; thr < threds; ++thr)
	{

		assert(current_number_proc <= processors -  1);
		bound_t* bd = nullptr;
		bd = new bound_t(save_start, (thr == threds - 1) ? bound.finish : save_start + step);
